...
std::string EspVideo::Explain(const std::string& question) {
    if (explain_url_.empty()) {
        return R"({"success":false,"message":"照片服务未配置"})";
    }

    // 创建局部的 JPEG 队列, 40 entries is about to store 512 * 40 = 20480 bytes of JPEG data
    QueueHandle_t jpeg_queue = xQueueCreateWithCaps(
        40, sizeof(JpegChunk), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (jpeg_queue == nullptr) {
        ESP_LOGE(TAG, "Failed to create JPEG queue");
        return R"({"success":false,"message":"JPEG队列内存不足"})";
    }
    ...
    auto* encoder_context = static_cast<EncoderTaskContext*>(heap_caps_malloc(
        sizeof(EncoderTaskContext), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (encoder_context == nullptr) {
        vQueueDeleteWithCaps(jpeg_queue);
        return R"({"success":false,"message":"JPEG任务内存不足"})";
    }
    *encoder_context = {this, jpeg_queue, xTaskGetCurrentTaskHandle()};

    // Use a native FreeRTOS task instead of std::thread: the latter always
    // requested an internal-RAM pthread stack and could terminate the device
    // when a voice session left only a fragmented internal heap.
    const BaseType_t encoder_result = xTaskCreatePinnedToCoreWithCaps(
        [](void* arg) {
        auto* context = static_cast<EncoderTaskContext*>(arg);
        EspVideo* video = context->video;
        QueueHandle_t queue = context->queue;
        TaskHandle_t owner = context->owner;
        heap_caps_free(context);
        uint16_t w = video->frame_.width ? video->frame_.width : 320;
        uint16_t h = video->frame_.height ? video->frame_.height : 240;
        v4l2_pix_fmt_t enc_fmt = video->frame_.format;
        bool ok = image_to_jpeg_cb(
            video->frame_.data, video->frame_.len, w, h, enc_fmt, 80,
        ...
        "camera_jpeg", 8192, encoder_context, 1, nullptr, 1,
        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (encoder_result != pdPASS) {
        heap_caps_free(encoder_context);
        vQueueDeleteWithCaps(jpeg_queue);
        return R"({"success":false,"message":"无法启动JPEG编码任务"})";
    }

    // Finish encoding before opening HTTP so the exact Content-Length is
    // known.  Chunked uploads could lose their terminating chunk when the
    // device's internal heap was highly fragmented, leaving aiohttp waiting
    // for the rest of the multipart body until both ends timed out.
    ...
    while (true) {
        ...
        jpeg_chunks.push_back(chunk);
    }
    ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
    vQueueDeleteWithCaps(jpeg_queue);

    auto free_jpeg_chunks = [&jpeg_chunks]() {
        for (auto& chunk : jpeg_chunks) {
            if (chunk.data != nullptr) {
    ...
    if (!saw_terminator || total_sent == 0) {
        ...
        return R"({"success":false,"message":"照片编码失败"})";
    }

    // An AI-initiated MCP capture already has a live WebSocket back to the
    // calling client. Return the JPEG through that existing connection rather
    // than opening a second HTTP socket, which is unreliable on the CoreS3
    // while its conversational WebSocket and camera buffers are both active.
    if (question == "__stackchan_mcp_inline__") {
        auto* jpeg_data = static_cast<unsigned char*>(heap_caps_malloc(
            total_sent, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
        if (jpeg_data == nullptr) {
            free_jpeg_chunks();
            return R"({"success":false,"message":"照片回传内存不足"})";
        }
        size_t offset = 0;
        for (const auto& chunk : jpeg_chunks) {
            memcpy(jpeg_data + offset, chunk.data, chunk.len);
            offset += chunk.len;
        }
        size_t encoded_capacity = 0;
        mbedtls_base64_encode(nullptr, 0, &encoded_capacity,
                              jpeg_data, total_sent);
        ...
        const int encode_result = mbedtls_base64_encode(
            ...
            &encoded_len, jpeg_data, total_sent);
        heap_caps_free(jpeg_data);
        free_jpeg_chunks();
        if (encode_result != 0 || encoded_len == 0) {
            return R"({"success":false,"message":"照片回传编码失败"})";
        }
        encoded.resize(encoded_len);
        ESP_LOGI(TAG, "Returning camera JPEG inline over MCP: jpeg=%u base64=%u",
                 (unsigned)total_sent, (unsigned)encoded_len);
        return std::string(
            R"({"success":true,"response":"照片已发送给当前助手","camera_mime_type":"image/jpeg","camera_image_base64":")") +
            encoded + R"("})";
    }
    ...
    if (network == nullptr) {
        free_jpeg_chunks();
        return R"({"success":false,"message":"网络服务不可用"})";
    }
    auto http = network->CreateHttp(3);
    if (http == nullptr) {
        free_jpeg_chunks();
        return R"({"success":false,"message":"无法创建照片上传任务"})";
    }
    ...
    const bool local_camera_action =
        question == "__stackchan_camera_look__" ||
        question == "__stackchan_manual_photo__" ||
        question == "__stackchan_face_recognize__" ||
        question.rfind("__stackchan_face_enroll__:", 0) == 0;
    const size_t content_length = local_camera_action
        ? total_sent
        : question_field.size() + file_header.size() + total_sent + multipart_footer.size();
    const char* local_action = question == "__stackchan_camera_look__"
        ? "look"
        : (question == "__stackchan_manual_photo__"
            ? "photo"
            : (question == "__stackchan_face_recognize__" ? "recognize" : "enroll_qiqi"));
    ...
    if (local_camera_action) {
        http->SetHeader("Content-Type", "image/jpeg");
        http->SetHeader("X-StackChan-Camera-Action", local_action);
    } else {
        http->SetHeader("Content-Type", "multipart/form-data; boundary=" + boundary);
    }
    http->SetHeader("Content-Length", std::to_string(content_length));
    ESP_LOGI(TAG, "Camera upload prepared: action=%s jpeg=%u content_length=%u chunks=%u",
             local_camera_action ? local_action : "vision",
             (unsigned)total_sent, (unsigned)content_length, (unsigned)jpeg_chunks.size());
    // An explicitly present empty content value selects raw writes instead of
    // request chunking; the body itself is streamed below from PSRAM buffers.
    http->SetContent(std::string{});
    bool opened = false;
    constexpr int kCameraConnectAttempts = 6;
    for (int attempt = 1; attempt <= kCameraConnectAttempts; ++attempt) {
        if (http->Open("POST", explain_url_)) {
            opened = true;
            break;
        }
        ESP_LOGW(TAG, "Camera service connect attempt %d/%d failed",
                 attempt, kCameraConnectAttempts);
        if (attempt < kCameraConnectAttempts) {
            // A CoreS3 station reconnect normally completes in roughly
            // 10-15 seconds. Keep the already encoded JPEG in PSRAM and give
            // Wi-Fi time to recover instead of immediately reporting that the
    ...
    if (!opened) {
        ...
        free_jpeg_chunks();
        return R"({"success":false,"message":"无法连接照片服务"})";
    }
    ...
    if (!local_camera_action) {
        ...
        const int header_written = question_written > 0
            ? http->Write(file_header.c_str(), file_header.size()) : -1;
        write_ok = question_written == (int)question_field.size() &&
                   header_written == (int)file_header.size();
        if (write_ok) {
            body_written += question_written + header_written;
        }
    }
    for (auto& chunk : jpeg_chunks) {
        if (write_ok) {
            const int chunk_written = http->Write(
                reinterpret_cast<const char*>(chunk.data), chunk.len);
            if (chunk_written != (int)chunk.len) {
                ESP_LOGE(TAG, "Camera JPEG write short: expected=%u actual=%d",
                         (unsigned)chunk.len, chunk_written);
                write_ok = false;
            } else {
                body_written += chunk_written;
            }
        }
    }
    free_jpeg_chunks();
    if (write_ok && !local_camera_action) {
        const int footer_written = http->Write(multipart_footer.c_str(), multipart_footer.size());
        write_ok = footer_written == (int)multipart_footer.size();
        if (write_ok) {
            body_written += footer_written;
        }
    }
    if (!write_ok) {
        http->Close();
        return R"({"success":false,"message":"照片发送中断"})";
    }

    ESP_LOGI(TAG, "Camera upload body sent: actual=%u expected=%u; waiting response",
             (unsigned)body_written, (unsigned)content_length);
    const int status_code = http->GetStatusCode();
    ESP_LOGI(TAG, "Camera upload response status=%d", status_code);
    if (local_camera_action) {
        if (status_code == 200) {
            const std::string encoded = http->GetResponseHeader("X-StackChan-Result");
            const bool success = http->GetResponseHeader("X-StackChan-Success") != "0";
            std::string decoded((encoded.size() * 3) / 4 + 4, '\0');
            size_t decoded_len = 0;
            const int decode_result = mbedtls_base64_decode(
                reinterpret_cast<unsigned char*>(decoded.data()), decoded.size(), &decoded_len,
                reinterpret_cast<const unsigned char*>(encoded.data()), encoded.size());
            http->Close();
            if (decode_result == 0 && decoded_len > 0) {
                decoded.resize(decoded_len);
                const char* key = success ? "response" : "message";
                return std::string("{\"success\":") + (success ? "true" : "false") +
                       ",\"" + key + "\":\"" + decoded + "\"}";
            }
            return R"({"success":false,"message":"照片服务返回结果无效"})";
        }
        http->Close();
        return status_code < 0
            ? R"({"success":false,"message":"照片服务响应超时"})"
            : R"({"success":false,"message":"照片上传失败"})";
    }
    std::string result = http->ReadAll();
    http->Close();
    if (status_code != 200) {
        ESP_LOGE(TAG, "Failed to upload photo, status code: %d, response: %s",
                 status_code, result.c_str());
        if (!result.empty()) {
            return result;
        }
        return R"({"success":false,"message":"照片上传失败"})";
    }
