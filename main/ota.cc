...
esp_err_t Ota::CheckVersion() {
    ...
    if (cJSON_IsObject(server_time)) {
        ...
        if (cJSON_IsNumber(timestamp)) {
            // A Unix timestamp is already a UTC instant. settimeofday() must
            // receive it unchanged; localtime_r() applies TZ=CST-8 later.
            // Applying timezone_offset here made China time eight hours fast
            // until the later SNTP synchronization corrected it.
            struct timeval tv;
