...
void CustomWakeWord::ReleaseRuntimeResources() {
    Stop();
    std::lock_guard<std::mutex> lock(input_buffer_mutex_);
    if (multinet_model_data_ != nullptr && multinet_ != nullptr) {
        multinet_->destroy(multinet_model_data_);
        multinet_model_data_ = nullptr;
        ESP_LOGI(TAG, "Released MultiNet runtime for low-memory mode");
    }
}

bool CustomWakeWord::RestoreRuntimeResources() {
    std::lock_guard<std::mutex> lock(input_buffer_mutex_);
    if (multinet_model_data_ != nullptr) return true;
    if (multinet_ == nullptr || mn_name_ == nullptr) return false;
    multinet_model_data_ = multinet_->create(mn_name_, duration_);
    if (multinet_model_data_ == nullptr) {
        ESP_LOGE(TAG, "Failed to restore MultiNet runtime");
        return false;
    }
    multinet_->set_det_threshold(multinet_model_data_, threshold_);
    ESP_LOGI(TAG, "Restored MultiNet runtime after low-memory mode");
    return true;
}
