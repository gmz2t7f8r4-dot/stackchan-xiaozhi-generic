...
void AudioService::SuspendWakeWordForMemory() {
    if (!wake_word_) return;
    EnableWakeWordDetection(false);
    wake_word_->ReleaseRuntimeResources();
}

bool AudioService::ResumeWakeWordAfterMemory() {
    if (!wake_word_) return true;
    if (!wake_word_->RestoreRuntimeResources()) return false;
