    ...
    if (camera) {
        AddTool("self.camera.take_photo",
            ...
            [camera](const PropertyList& properties) -> ReturnValue {
                ...
                auto& active_board = Board::GetInstance();
                if (!active_board.WaitForCameraShutter()) {
                    throw std::runtime_error("The user did not press the camera shutter in time");
                }
                try {
                    if (!camera->Capture()) {
                    ...
                    active_board.FinishCameraCapture(true);
                    return result;
                } catch (...) {
                    active_board.FinishCameraCapture(false);
                    throw;
                }
            });
    }
