...
bool Assets::LoadSrmodelsFromIndex(Assets* assets, cJSON* root) {
    ...
    // Some boards keep the same model blob in a dedicated `model` partition.
    // Prefer that copy when assets intentionally omit srmodels.bin to leave
    // room for a high-coverage runtime font.
    if (assets->models_list_ != nullptr) {
        esp_srmodel_deinit(assets->models_list_);
        assets->models_list_ = nullptr;
    }
    assets->models_list_ = esp_srmodel_init("model");
    if (assets->models_list_ != nullptr) {
        Application::GetInstance().GetAudioService().SetModelsList(assets->models_list_);
        ESP_LOGI(TAG, "Loaded speech models from dedicated model partition");
        if (need_delete_root) {
            cJSON_Delete(root);
        }
        return true;
    }
    ESP_LOGE(TAG, "Failed to load speech models from dedicated model partition");
