...
def get_emoji_collection_path(default_emoji_collection, xiaozhi_fonts_path, project_root=None):
    ...
    # Project-local collections are used by boards with a bespoke avatar.  Keep
    # them outside the managed xiaozhi-fonts component so component updates do
    # not delete the device's own artwork.
    if project_root:
        project_emoji_path = os.path.join(
            project_root, 'main', 'assets', 'emoji', default_emoji_collection)
        if os.path.exists(project_emoji_path):
...
def main():
    ...
    parser.add_argument('--exclude_srmodels', action='store_true',
                        help='Keep speech models in the dedicated model partition instead of duplicating them in assets')
    ...
    if args.exclude_srmodels:
        print("  SR models: using dedicated model partition (excluded from assets)")
        wakenet_model_paths = []
        multinet_model_paths = []
    ...
    if custom_wake_word_config and (multinet_model_paths or args.exclude_srmodels):
